...
public class WelcomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnNext.setOnClickListener(v -> {
            currentPage++;
            if (currentPage >= pages.length) finishOnboarding();
            else showPage(currentPage);
        });
        btnSkip.setOnClickListener(v -> finishOnboarding());
    }
    private void showPage(int page) {
        ((TextView)findViewById(R.id.tv_welcome_title)).setText(pages[page][0]);
        ((TextView)findViewById(R.id.tv_welcome_desc)).setText(pages[page][1]);
    }
    private void finishOnboarding() {
        getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit().putBoolean(KEY_DONE, true).apply();
        startActivity(new Intent(this, ChildActivity.class));
        finish();
    }
}
